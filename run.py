...
class Twitch:
  def fetch_followed_channels(self, username):
    ...
    try:
      ...
      self.f = urllib.urlopen("https://api.twitch.tv/kraken/users/{0}/follows/channels?client_id=fardf5u3616a11v2v9ya6otkr35vo82&direction=DESC&limit=100&offset=0&sortby=created_at".format(username))
      ...
      for page in range(0, self.pages + 1):
        if page != 0:
          self.f = urllib.urlopen("https://api.twitch.tv/kraken/users/{0}/follows/channels?client_id=fardf5u3616a11v2v9ya6otkr35vo82&direction=DESC&limit=100&offset={1}&sortby=created_at".format(username, (page * 100)))
          self.data = json.loads(self.f.read())
  ...
  def fetch_live_streams(self, channels):
    ...
    try:
      ...
      for page in range(0, self.pages + 1):
        ...
        self.f = urllib.urlopen("https://api.twitch.tv/kraken/streams?client_id=fardf5u3616a11v2v9ya6otkr35vo82&channel={0}".format(','.join(self.channels_offset)))
        self.data = json.loads(self.f.read())
